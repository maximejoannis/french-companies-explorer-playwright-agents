...
async function downloadConfigured(
  ...
  const query = await search.queryInput.inputValue();
  const slug = query
    ...
    .replace(/^-|-$/g, '')
    .slice(0, 40);
  return downloadText(page, search.confirmExportButton, `entreprises-${scope}-${slug}.${format}`);
}
...
test('TC-EXPORT-001 @regression exporte la représentation JSON normalisée', async ({ page }) => {
  ...
  await expect(search.openExportButton).toBeVisible();
  await expect(search.openExportButton).toBeEnabled();
  const storageBeforeExport = await localStorageSnapshot(page);
  const requestsBeforeExport = apiRequests.length;

  const text = await downloadConfigured(page, search, 'json');
...
test('TC-EXPORT-002 @regression produit un CSV structuré et correctement associé', async ({
  ...
  const text = await downloadConfigured(page, search, 'csv');
  ...
  expect(rows.slice(1)).toEqual(
    mockedCompanies.map((company) => [
      company.siren,
      company.nom_complet,
      company.etat_administratif === 'A' ? 'En activité' : 'Cessée',
...
test('TC-EXPORT-004 @regression exporte uniquement la page courante dans l’ordre du tri', async ({
  ...
  const jsonText = await downloadConfigured(page, search, 'json');
  const csvText = await downloadConfigured(page, search, 'csv');
...
test('TC-EXPORT-005 @regression refuse un export vide sans créer de fichier', async ({ page }) => {
  ...
  await search.configureExport('json');
...
test('TC-EXPORT-006 @regression BUG-013 refuse d’exporter une collection précédente', async ({
  ...
  expectNoApiWrites(apiRequests);
  expect(exported.map(({ siren }) => siren)).toEqual([betaCompany.siren]);
});
