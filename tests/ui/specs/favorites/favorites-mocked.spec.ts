...
test('TC-FAVORITES-006 @accessibility expose le nom et lՎtat du favori', async ({ page }) => {
  ...
  await expect(button).toHaveAccessibleName(`Ajouter ${alphaCompany.nom_complet} aux favoris`);
  await expect(button).toHaveAttribute('aria-pressed', 'false');
  await button.click();
  await expect(button).toHaveAccessibleName(`Retirer ${alphaCompany.nom_complet} des favoris`);
