...
test.beforeEach(async ({ page }) => {
  await page.route('https://geo.api.gouv.fr/communes**', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      json: [{ nom: 'Lyon', code: '69123', codesPostaux: ['69001'] }],
    }),
  );
...
test('TC-SAVED-006 @regression supprime uniquement la sauvegarde ciblée', async ({ page }) => {
  ...
  await expect(search.savedSearchDeleteButton('Nom Bêta', 'Bêta suppression')).toHaveAccessibleName(
    'Supprimer la recherche Nom Bêta',
  );
...
test('TC-SAVED-008 @regression BUG-011 refuse un nom composé uniquement d’espaces', async ({
  ...
  for (const invalidName of ['', '   ', '\t\t']) {
    await answerSavePrompt(page, search, invalidName);
  }
