...
const GEO_PATTERN = 'https://geo.api.gouv.fr/communes**';
...
test.beforeEach(async ({ page }) => {
  await page.route(GEO_PATTERN, (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      json: [{ nom: 'Paris', code: '75056', codesPostaux: ['75001'] }],
    }),
  );
...
test('TC-FILTERS-005 @positive transmet chaque contrôle sous le bon paramètre API', async ({
  ...
  await page.route(API_PATTERN, async (route) => {
    ...
    if (url.searchParams.get('minimal') === 'true') return mockJson(route, { results: [] });
    requests.push(url);
  ...
  expect(requests[1].searchParams.get('code_commune')).toBe('75056');
...
test('TC-FILTERS-007 conserve et présente les filtres actifs après succès', async ({ page }) => {
  ...
  await expect(
    search.activeFilters.getByRole('button', { name: /Supprimer le filtre Code postal/ }),
  ).toBeVisible();
  await expect(
    search.activeFilters.getByRole('button', { name: /Supprimer le filtre Commune/ }),
  ).toBeVisible();
  await expect(
    search.activeFilters.getByRole('button', { name: /Supprimer le filtre État/ }),
  ).toBeVisible();
});
