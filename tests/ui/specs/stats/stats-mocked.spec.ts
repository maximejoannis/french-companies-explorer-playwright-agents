...
async function expectStats(search: SearchPage, expected: ExpectedStats) {
  ...
  await expect(search.statsBlock('En activité')).toHaveText(
    new RegExp(
      `En activité\\s*${expected.active}\\s*${expected.closed} cessée\\(s\\) · ${expected.unknown ?? expected.displayed - expected.active - expected.closed} non renseigné\\(s\\)$`,
    ),
  );
...
test('TC-STATS-003 @positive recalcule les statistiques sur la réponse filtrée', async ({
  ...
  await page.route(API_PATTERN, async (route) => {
    ...
    if (request.searchParams.get('minimal') === 'true') {
      await mockJson(route, { results: [] });
      return;
    }
